...
export default function VisualEditorPage() {
    ...
    const handleIframeLoad = async () => {
      try {
        ...
        try {
          ...
          console.log('[AI Autopilot] CORS restriction on iframe URL');
          return;
        }
        ...
        if (iframeHref.includes('/api/visual-proxy?url=')) {
          try {
            ...
            console.error('[AI Autopilot] Failed to parse proxy URL:', e);
          }
        }
        ...
        console.log('[AI Autopilot] Navigation detected:', path);
        setCurrentPath(path);
        ...
        console.error('[AI Autopilot] Navigation tracking error:', error);
      }
    };
  ...
  useEffect(() => {
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (event.data.type === 'IMAGE_CLICKED') {
        console.log('[AI Autopilot] Image clicked:', event.data.data);
      ...
      if (event.data.type === 'ELEMENT_CLICKED') {
        console.log('[AI Autopilot] Element clicked:', event.data.data);
        ...
        try {
          ...
          console.error('[AI Autopilot] Error creating slot:', err);
  ...
  const fetchImageSize = async (imageUrl: string) => {
    try {
      ...
      console.log('[AI Autopilot] Could not fetch image size');
    }
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.text_regions && data.text_regions.length > 0) {
          ...
          console.log('[AI Autopilot] Detected text:', detectedTexts);
        } else {
      ...
      console.log('[AI Autopilot] Could not detect text from image');
  ...
  const analyzeImageToPrompt = async (imageUrl: string) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('[AI Autopilot] Generated prompt:', data.prompt);
      } else {
        setImagePrompt('A professional marketing image with gradient background');
      }
    } catch (err) {
      console.log('[AI Autopilot] Could not analyze image');
      setImagePrompt('A professional marketing image with gradient background');
    } finally {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{
        ...
        <h1 style={{ fontSize: '20px', fontWeight: 'bold', margin: 0 }}>AI Autopilot</h1>
